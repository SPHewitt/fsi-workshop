...
//           - fluidPatchPointsDisplPrev;
        ...
        bool rbfMeshMotionSolver =
             fluidMesh().objectRegistry::foundObject<RBFMeshMotionSolver>
             (
                 "dynamicMeshDict"
             );
		...
		Field<vectorField> patches( fluidMesh().boundaryMesh().size(), vectorField( 0 ) );

                //const labelList& movingMeshPoints = fluidMesh().boundaryMesh()[fluidPatchIndex()].meshPoints();

                vectorField motion(fluidMesh().boundaryMesh()[fluidPatchIndex()].size(), vector::zero); 

		patches[fluidPatchIndex()] = motion;
...
Foam::scalar Foam::fluidSolidInterface::updateResidual()
{
    ...
    Info << "residual Sum" << endl;

    Info << gSum(residual()) << endl;
