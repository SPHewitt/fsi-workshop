...
void Foam::fluidSolidInterface::calcGgiInterpolator() const
{
    ...
    Info << "Create extended GGI zone-to-zone interpolator" << endl;
     ...
     solidMesh().faceZones()[solidZoneIndex_]().writeVTK("solidZone");
...
//            currentSolidZonePatch(), // THIS WAS THE ORIGINAL
...
// HEWITT I CHANGED BELOW
       vectorField solidZonePoints_ =
//            currentSolidZonePoints(); //THIS WAS ORIGINAL
    ...
    minEdgeLengthPtr_(NULL)
{
    ...
    if (found("refLength"))
    {
        refLength_ = readScalar(lookup("refLength"));
    }
    else
    {
        refLength_ = 1.0;
...
void Foam::fluidSolidInterface::initializeFields()
{
    ...
    solidZoneDispPrev_=solidZonePointsDispl_;
...
//	  fileName outputFile("Residual.txt");
...
//           - fluidPatchPointsDisplPrev;
        ...
        if (feMotionSolver)
        {
            ...
            motionUFluidPatch ==
                ...
                (
                (fluidPatchPointsDispl- fluidPatchPointsDisplPrev)
...
//                );
 ...
 //                refCast<const dynamicMotionSolverFvMesh>(fluidMesh_);
        ...
        {
          pointVectorField& displacementU =
              const_cast<pointVectorField&>
              (
                  fluidMesh().objectRegistry::
                  lookupObject<pointVectorField>
          ...
          fixedValuePointPatchVectorField& displacmentUFluidPatch =
              refCast<fixedValuePointPatchVectorField>
              (
                  displacementU.boundaryField()[fluidPatchIndex()]
              );

          displacmentUFluidPatch ==
          (
              fluidPatchPointsDispl
            - fluidPatchPointsDisplPrev
          );
...
Foam::scalar Foam::fluidSolidInterface::updateResidual()
{
    ...
    Info << "Current fsi residual: " << residualNorm << endl;

    interfacePointsDisplPrev_ = interfacePointsDispl_; // d(n,k-1)

    interfacePointsDispl_ = solidZonePointsDispl();    // d(n,k)

    // Breuer 2015
    // d(n,k) - d(n,k-1) / d(n,k) - d(n-1)
    //Info << "Dn-1" << endl;
    //Info << sum(solidZoneDispPrev_) << endl;

    //Pout << "Dnk-1" << endl;
    //Pout << sum(interfacePointsDisplPrev_) << endl;

    //Pout << "Dnk" << endl;
    //Pout << sum(interfacePointsDispl_) << endl;

    vectorField intTotDispl =
        interfacePointsDispl_ + solidZonePointsTotDispl;

    scalar intTotDisplNorm = ::sqrt(sum(magSqr(intTotDispl)));
    ...
    scalar residualNorm_3 = 1;

    if ((outerCorr_ > 0) && predictor())
    {
        vectorField nom = interfacePointsDispl_ - interfacePointsDisplPrev_;
        scalar nomRes = ::sqrt(sum(magSqr(nom)));
        residualNorm_3 = nomRes/((::sqrt(nom.size()) * refLength_)+ SMALL);
    }
    else if(outerCorr_ > 1)
    {
        vectorField nom = interfacePointsDispl_ - interfacePointsDisplPrev_;
        scalar nomRes = ::sqrt(sum(magSqr(nom)));
        residualNorm_3 = nomRes/((::sqrt(nom.size()) * refLength_)+ SMALL);
    ...
    // BUG residualNorm_2 not always same on all processors
    // Use the master processor values
    if(!Pstream::master())
    {
        residualNorm_3 = 0.0;
    }

    reduce(residualNorm_3,sumOp<scalar>());

    Info << "Normalised fsi residual: " << residualNorm_3 << endl;


    scalar alterResidual = min(residualNorm_3,residualNorm_2);

    return min(residualNorm,alterResidual);
    //return min(residualNorm_2, residualNorm);
}
