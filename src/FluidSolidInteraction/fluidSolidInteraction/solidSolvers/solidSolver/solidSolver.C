    ...
    labelListList& globalToLocalFaceZonePointMap =
       ...
       labelListIOList globalToLocal
        (
            ...
            labelListList(globalFaceZones().size())
        );
    
    if (header.headerOk())
    {
        // Read globalToLocalFaceZonePointMap from file
        Info << "Reading In globalToLocalFaceZonePointMap" << endl;

        // Note this only currently works for one global 
        // face zone
        globalToLocalFaceZonePointMap=globalToLocal;
    }
    else
    {
        Info << "Creating In globalToLocalFaceZonePointMap" << endl;
        ...
        //             }
            ...
            globalToLocal=globalToLocalFaceZonePointMap;
            globalToLocal.write();    
    
    }

//    fileName outputFile("globalToLocalFaceZonePointMap.txt");
//    OFstream os(runTime().path()/runTime().constant()/outputFile);

//    os << "globalToLocalFaceZonePointMap\n";
//    os << "labelListList {" << globalToLocalFaceZonePointMap << "};" << endl;
