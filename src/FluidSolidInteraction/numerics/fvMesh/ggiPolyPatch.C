        ...
        patchToPatchPtr_ = 
            ...
            (
               ...
               -separation(),   // Slave-to-master separation. Bug fix
                ...
                reject_         // Quick rejection algorithm,
                                // default BB_OCTREE
                                    // N_SQUARED BB_OCTREE AABB 
                                    // THREE_D_DISTANCE  
                                    // Octree search, MB.
            );


        Info << "Checking master-to-slave face interpolator" << endl;
        {
            vectorField masterPatchFaceCentres = 
                vectorField
                (
                    faceCentres()
                );

            vectorField masterZoneFaceCentres
                (
                    zone()().size(), 
                    vector::zero
                );
            
            const label masterPatchStart = start();
            
            forAll (masterPatchFaceCentres, i)
            {
                masterZoneFaceCentres
                [
                    zone().whichFace
                    (
                        masterPatchStart + i
                    ...
                    masterPatchFaceCentres[i];
            }

            // Parallel data exchange: 
            // collect faceCentres field on all processors
            reduce(masterZoneFaceCentres, sumOp<vectorField>());

            vectorField slaveZoneFaceCentres =
                patchToPatchPtr_->masterToSlave
                (
                    masterZoneFaceCentres
                );
            
            vectorField slavePatchFaceCentres
                (
                    shadow().size(), 
                    vector::zero
                );

            const label slavePatchStart =  shadow().start();
            
            forAll(slavePatchFaceCentres, i)
            {
                slavePatchFaceCentres[i] =
                    slaveZoneFaceCentres
                    [
                        shadow().zone()
            ...
            scalar maxDist = gMax
                (
                    mag
                    (
                        slavePatchFaceCentres
                      - shadow().faceCentres()
                    )
                );

            Info << "Master-to-slave face interpolation error: " << maxDist 
                << endl;
    }

        Info << "Checking slave-to-master face interpolator" << endl;
        {
            vectorField slavePatchFaceCentres = 
                vectorField
                (
            ...
            // Parallel data exchange: 
            // collect faceCentres field on all processors
            reduce(slaveZoneFaceCentres, sumOp<vectorField>());

            vectorField masterZoneFaceCentres =
                patchToPatchPtr_->slaveToMaster
                (
                    slaveZoneFaceCentres
                );
            ...
            scalar maxDist = gMax
                (
                    mag
                    (
                        masterPatchFaceCentres
                      - faceCentres()
                    )
                );

            Info << "Slave-to-Master face interpolation error: " << maxDist 
                << endl;
    }
...
const Foam::extendedGgiZoneInterpolation& Foam::ggiPolyPatch::patchToPatch() const
{
    if (master())
    {
        if (!patchToPatchPtr_)
...
void Foam::ggiPolyPatch::initMovePoints(const pointField& p)
{
    ...
    Info << "ggi interpolation is not updated" << endl;
