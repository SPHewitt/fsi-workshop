...
) const
{
    ...
    while (changedFaces.size())
    {
        ...
        forAll(patches, patchI)
        {
            ...
            else if (isA<cyclicGgiPolyPatch>(pp))
            {
                const cyclicGgiPolyPatch& cyclicGgi =
                    refCast<const cyclicGgiPolyPatch>(pp);

                if (cyclicGgi.master())
                {
                    label start = cyclicGgi.start();
                    label slaveStart = cyclicGgi.shadow().start();

                    labelField masterFaceRegion(cyclicGgi.size(), -1);
                    labelField slaveFaceRegion(cyclicGgi.shadow().size(), -1);

                    forAll(masterFaceRegion, faceI)
                    {
                        masterFaceRegion[faceI] = faceRegion[start+faceI];
                    }

                    forAll(slaveFaceRegion, faceI)
                    {
                        slaveFaceRegion[faceI] = faceRegion[slaveStart+faceI];
                    }

                    labelField slaveFaceRegionAtMaster =
                        cyclicGgi.interpolate(slaveFaceRegion);

                    labelField masterFaceRegionAtSlave =
                        cyclicGgi.shadow().interpolate(masterFaceRegion);

                    forAll(masterFaceRegion, faceI)
                    {
                        if (masterFaceRegion[faceI] < 0)
                        {
                            if (slaveFaceRegionAtMaster[faceI]>=0)
                            {
                                faceRegion[start+faceI] = slaveFaceRegionAtMaster[faceI];
                                newChangedFaces.append(start+faceI);
                            }
                        }
                    }
                    ...
                    forAll(slaveFaceRegion, faceI)
                    {
                        if (slaveFaceRegion[faceI] < 0)
                        {
                            if (masterFaceRegionAtSlave[faceI]>=0)
                            {
                                faceRegion[slaveStart+faceI] = masterFaceRegionAtSlave[faceI];
                                newChangedFaces.append(slaveStart+faceI);
                            }
                        }
                    }
                }

//                 Info << "CyclidGgi is not implemented" << endl;
            }
        }
